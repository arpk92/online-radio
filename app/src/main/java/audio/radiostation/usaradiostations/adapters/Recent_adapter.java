...
public class Recent_adapter extends RecyclerView.Adapter<Recent_adapter.MyviewHolder> {
    List<Model_class> mylist = new ArrayList();
    ArrayList<Model_class> listmain = new ArrayList();

    Context context;
    public Recent_adapter(FragmentActivity fragmentActivity, List<Model_class> mylist) {
    ...
    public MyviewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new Recent_adapter.MyviewHolder( itemView );
    }


    @Override
    public void onBindViewHolder(@NonNull final Recent_adapter.MyviewHolder holder, final int i) {
        ...
        holder.menu.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (Favorite_save_Prefrence.getInstance(context).isFavorite(mylist.get(i))) {
                    ...
                    popup.inflate(R.menu.recentmenuadap2);
                } else {
                    popup = new PopupMenu(context, holder.menu);
                    popup.inflate(R.menu.recentmenuadap);
                }
...
//                                    Toast.makeText(context, "Added to favorites", Toast.LENGTH_SHORT).show();
                                ...
                                case R.id.rem_rec:
                                if (Recent_list.getInstance(context).isFavorite(mylist.get(i))) {
                                    Recent_list.getInstance(context).removeFavorite(context, mylist.get(i));

                                    ((Main)context).customtoast("Removed from Recent");
                                    mylist.remove(i);
                                    notifyItemChanged( i );
                                    notifyDataSetChanged();
                                    notifyDataSetChanged();
